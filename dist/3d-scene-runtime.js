// Name: TurboWarp-3D-Scene-Runtime
// ID: kubohiroya3dsceneruntime
// Description: Applies a 3D/AR scene description by dispatching its call plan to turbowarp-aframe and turbowarp-ar.
// By: Hiroya Kubo
// License: MPL-2.0
...
(function (Scratch) {
  ...
  var extensionConfig = {
  	id: "kubohiroya3dsceneruntime",
  	slug: "3d-scene-runtime",
  	name: "TurboWarp-3D-Scene-Runtime",
  	description: "Applies a 3D/AR scene description by dispatching its call plan to turbowarp-aframe and turbowarp-ar.",
  	author: "Hiroya Kubo",
  	license: "MPL-2.0",
  	unsandboxed: true,
  	docsURI: "https://kubohiroya.github.io/turbowarp-3d-scene-runtime/",
  	blockIconURI: "data:image/svg+xml;base64,"
  };
  //#endregion
  //#region node_modules/.pnpm/@kubohiroya+turbowarp-scene-graph@0.2.0/node_modules/@kubohiroya/turbowarp-scene-graph/dist/scene-graph.js
  var defaultOptions = Object.freeze({
  	layer: "above-stage",
  	mode: "3d"
  });
  var rootId = "scene";
  var nodeKeys = /* @__PURE__ */ new Set([
  	"attributes",
  	"children",
  	"class",
  	"classes",
  ...
  var documentKeys = /* @__PURE__ */ new Set([
  	"formatVersion",
  	"options",
  	"root"
  ]);
  var optionKeys = /* @__PURE__ */ new Set(["layer", "mode"]);
  function validateSceneGraphDocument(value) {
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("Scene graph document must be an object.");
  	const document = value;
  	for (const key of Object.keys(document)) if (!documentKeys.has(key)) throw new TypeError(`Scene graph document ${key} is not supported.`);
  	if (document["formatVersion"] !== 1) throw new TypeError("Scene graph document formatVersion must be 1.");
  	validateSceneGraphOptions(document["options"]);
  	validateSceneGraphNode(document["root"], "root");
  }
  function validateSceneGraphNode(value, path = "node") {
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError(`Scene graph ${path} must be an object.`);
  	const node = value;
  	for (const key of Object.keys(node)) if (!nodeKeys.has(key)) throw new TypeError(`Scene graph ${path}.${key} is not supported.`);
  	validateOptionalString$1(node["type"], `${path}.type`);
  	validateOptionalString$1(node["id"], `${path}.id`);
  	...
  	if (!Array.isArray(node["children"])) throw new TypeError(`Scene graph ${path}.children must be an array.`);
  	node["children"].forEach((child, index) => validateSceneGraphNode(child, `${path}.children[${index}]`));
  }
  function normalizeSceneGraphDocument(document) {
  ...
  function validateSceneGraphOptions(value) {
  	if (value === void 0) return;
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("Scene graph document options must be an object.");
  	const options = value;
  	for (const key of Object.keys(options)) if (!optionKeys.has(key)) throw new TypeError(`Scene graph document options.${key} is not supported.`);
  	validateOptionalString$1(options["layer"], "options.layer");
  	validateOptionalString$1(options["mode"], "options.mode");
  }
  function appendNodeCalls(calls, node, parentId) {
  	const id = normalizeId$1(node.id ?? "");
  	...
  	for (const className of classTokens(node)) calls.push({
  		extension: "turbowarp-aframe",
  		opcode: "addClass",
  		args: {
  			CLASS: className,
  	...
  	for (const [key, value] of Object.entries(node.data ?? {})) calls.push({
  		extension: "turbowarp-aframe",
  		opcode: "setData",
  		args: {
  			SELECTOR: selector,
  			KEY: key,
  			VALUE: stringifySceneGraphValue(value)
  		}
  	});
  	for (const [name, value] of Object.entries(node.attributes ?? {})) calls.push({
  		extension: "turbowarp-aframe",
  		opcode: "setAttribute",
  		args: {
  			SELECTOR: selector,
  ...
  function classTokens(node) {
  	return [...Array.isArray(node.class) ? node.class : String(node.class ?? "").split(/\s+/), ...node.classes ?? []].map((value) => value.trim()).filter((value) => value.length > 0);
  }
  function normalizeChildNode(node, parentId, index) {
  	const next = cloneNode(node);
  	next.id = normalizeId$1(next.id ?? `${parentId}-${normalizeId$1(next.type ?? "node") || "node"}-${index + 1}`);
  ...
  function visit$2(node, visitor) {
  	visitor(node);
  	for (const child of node.children ?? []) visit$2(child, visitor);
  }
  function cloneNode(node) {
  	return {
  		...
  		...node.children === void 0 ? {} : { children: node.children.map(cloneNode) }
  	};
  }
  function validateClassList(value, path) {
  ...
  function validateOptionalString$1(value, path) {
  	if (value === void 0 || typeof value === "string") return;
  ...
  function validateRecord(value, path) {
  	...
  	for (const [key, child] of Object.entries(value)) {
  		...
  		if (child === void 0) throw new TypeError(`Scene graph ${path}.${key} must not be undefined.`);
  		validateSceneGraphValue(child, `${path}.${key}`);
  	}
  }
  function validateSceneGraphValue(value, path) {
  ...
  function validateSceneGraphScalar(value, path) {
  	if (typeof value !== "string" && typeof value !== "number" && typeof value !== "boolean" && value !== null) throw new TypeError(`Scene graph ${path} must be a scalar value.`);
  }
  function stringifySceneGraphValue(value) {
  	...
  	if (keys.length > 0 && keys.every((key) => [
  		...
  		"z",
  		"w"
  	].filter((key) => Object.hasOwn(objectValue, key)).map((key) => stringifySceneGraphScalar(objectValue[key] ?? null)).join(" ");
  	return keys.sort().map((key) => `${key}: ${stringifySceneGraphScalar(objectValue[key] ?? null)}`).join("; ");
  }
  function stringifySceneGraphScalar(value) {
  ...
  */
  ...
  //#endregion
  //#region node_modules/.pnpm/@kubohiroya+turbowarp-ar@0.3.0/node_modules/@kubohiroya/turbowarp-ar/dist/plan.js
  var AR_SCENE_LAYERS = Object.freeze(["above-stage", "below-stage"]);
  function normalizeARSceneControl(control) {
  	validateARSceneControl(control);
  	return {
  		cameraId: normalizeId(control.cameraId ?? "default") || "default",
  		layer: control.layer ?? "above-stage",
  		targets: [...control.targets ?? []]
  	};
  }
  function createTurboWarpARScenePlan(control) {
  ...
  function validateARSceneControl(value) {
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("TurboWarp AR scene control must be an object.");
  	const control = value;
  	for (const key of Object.keys(control)) if (key !== "cameraId" && key !== "layer" && key !== "targets") throw new TypeError(`TurboWarp AR scene control ${key} is not supported.`);
  	...
  	if (!Array.isArray(targets)) throw new TypeError("TurboWarp AR scene control targets must be an array.");
  	targets.forEach((target, index) => {
  		validateARTargetBinding(target, `ar.targets[${index}]`);
  	});
  }
  function validateARTargetBinding(value, path) {
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError(`TurboWarp AR ${path} must be an object.`);
  	const target = value;
  	for (const key of Object.keys(target)) if (key !== "targetId" && key !== "selector") throw new TypeError(`TurboWarp AR ${path}.${key} is not supported.`);
  	if (typeof target["targetId"] !== "string" || target["targetId"].trim().length === 0) throw new TypeError(`TurboWarp AR ${path}.targetId must be a non-empty string.`);
  	if (typeof target["selector"] !== "string" || target["selector"].trim().length === 0) throw new TypeError(`TurboWarp AR ${path}.selector must be a non-empty string.`);
  }
  function validateLayer(value) {
  	if (value === void 0) return;
  	validateOptionalString(value, "ar.layer");
  	if (!AR_SCENE_LAYERS.includes(value)) throw new TypeError(`TurboWarp AR scene control ar.layer must be one of ${AR_SCENE_LAYERS.join(", ")}.`);
  }
  ...
  //#endregion
  //#region node_modules/.pnpm/@kubohiroya+turbowarp-scene-graph@0.2.0/node_modules/@kubohiroya/turbowarp-scene-graph/dist/kamishibai-extension.js
  function validateKamishibai3DSceneExtension(value) {
  	if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("Kamishibai 3D scene extension must be an object.");
  	const extension = value;
  	for (const key of Object.keys(extension)) if (key !== "scene3d" && key !== "ar") throw new TypeError(`Kamishibai 3D scene extension ${key} is not supported.`);
  	if (extension["scene3d"] !== void 0) validateSceneGraphDocument(extension["scene3d"]);
  	if (extension["ar"] !== void 0) validateARSceneControl(extension["ar"]);
  }
  function normalizeKamishibai3DSceneExtension(extension) {
  	validateKamishibai3DSceneExtension(extension);
  	const normalized = {};
  	if (extension.scene3d !== void 0) normalized.scene3d = normalizeSceneGraphDocument(extension.scene3d);
  	if (extension.ar !== void 0) normalized.ar = normalizeARSceneControl(extension.ar);
  	return normalized;
  }
  /**
  ...
  */
  function createTurboWarpExtensionPlan(extension) {
  	const normalized = normalizeKamishibai3DSceneExtension(extension);
  	const calls = [];
  	if (normalized.scene3d !== void 0) calls.push(...createAFrameSceneGraphPlan(normalized.scene3d));
  	if (normalized.ar !== void 0) calls.push(...createTurboWarpARScenePlan(normalized.ar));
  	return calls;
  }
  var block_definitions_default = {
  	extensionName: "TurboWarp-3D-Scene-Runtime",
  	blocks: [
  		{
  			"opcode": "applyScene",
  			"blockType": "COMMAND",
  			"text": "apply 3D scene [SOURCE]",
  			"description": "Validates a YAML or JSON scene description and builds it by dispatching its call plan to turbowarp-aframe and turbowarp-ar.",
  			"arguments": { "SOURCE": {
  				"type": "STRING",
  				"defaultValue": "formatVersion: 1"
  			} }
  		},
  		{
  			"opcode": "sceneStatus",
  			"blockType": "REPORTER",
  			"text": "3D scene status",
  			"description": "Returns idle, applying, ready, or error.",
  			"arguments": {}
  		},
  		{
  			"opcode": "sceneError",
  			"blockType": "REPORTER",
  			"text": "3D scene error",
  			"description": "Returns the message from the last failed apply, or an empty string.",
  			"arguments": {}
  		},
  		{
  			"opcode": "isSceneReady",
  			"blockType": "BOOLEAN",
  			"text": "3D scene is ready?",
  			"description": "Reports whether the last apply finished without an error.",
  			"arguments": {}
  		},
  		{
  			"opcode": "whenSceneApplied",
  			"blockType": "HAT",
  			"text": "when 3D scene applied",
  			"description": "Fires once each time a scene finishes being applied.",
  			"arguments": {}
  		}
  	]
  };
  ...
  var AR_CAPABILITY_KEY = "turbowarpARCapability";
  var AFRAME_PORT_METHODS = [
  	"createScene",
  	"createNode",
  	"addClass",
  	"setData",
  ...
  var TurboWarp3DSceneRuntimeExtension = class {
  	constructor() {
  		this.status = "idle";
  		this.lastError = "";
  	...
  	snapshot() {
  		return {
  			...
  			pendingAppliedEvents: this.appliedEvents
  		};
  	}
  	/**
  	...
  	*/
  	planFor(source) {
  		...
  		if (typeof value !== "object" || value === null || Array.isArray(value)) throw new TypeError("3D scene description must be a YAML or JSON object.");
  		const keys = Object.keys(value);
  		return createTurboWarpExtensionPlan(keys.includes("scene3d") || keys.includes("ar") ? value : { scene3d: value });
  	}
  	async dispatch(calls) {
  		...
  		for (const call of calls) if (call.extension === "turbowarp-aframe") await this.dispatchAFrame(aframe, call);
  		else await this.dispatchAR(ar, call);
  	}
  	async dispatchAFrame(port, call) {
  		switch (call.opcode) {
  			case "createScene":
  	...
  	requireAFramePort() {
  		const port = this.capability(AFRAME_CAPABILITY_KEY, AFRAME_PORT_METHODS);
  		if (port === null) throw new Error("turbowarp-aframe 0.7.0 or newer must be loaded before a 3D scene can be applied.");
  		return port;
  	}
  	requireARPort() {
  		const port = this.capability(AR_CAPABILITY_KEY, AR_PORT_METHODS);
  		if (port === null) throw new Error("turbowarp-ar 0.4.0 or newer must be loaded before a scene with an ar fragment can be applied.");
  		return port;
  	}
  	capability(key, methods) {
  		...
  		for (const method of methods) if (typeof port[method] !== "function") return null;
  		return candidate;
  	}
