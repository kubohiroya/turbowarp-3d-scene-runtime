...
function checkPolicy() {
  ...
  if (policy.productName !== 'TurboWarp-3D-Scene-Runtime') {
...
function checkPackageMetadata() {
  ...
  if (packageMetadata.repository?.url !== 'git+https://github.com/kubohiroya/turbowarp-3d-scene-runtime.git') {
    errors.push('package.json repository.url must point to the current repository');
  }
  if (packageMetadata.bugs?.url !== 'https://github.com/kubohiroya/turbowarp-3d-scene-runtime/issues') {
...
async function checkPackContents() {
  ...
  if (!files.has('dist/3d-scene-runtime.js')) {
